...
public class WMethod{
   ...
   */
   public static void main(String [] args){
     ...
     for (String tc : tests) {
    	 Utilities.runFSM(FSM, 1, tc, "");
     }

     // Generate a test class
     // Create file to store test cases
      try {
      	 PrintWriter writer = new PrintWriter("../project5/src/jamesbond/JamesBondTest.java", "UTF-8");
         writer.println("package jamesbond;");
      	 writer.println("import org.junit.Test;");
      	 writer.println("import static org.junit.Assert.*;");
      	 writer.println("public class JamesBondTest {");
         writer.println("JamesBond Bond = new JamesBond();");
         int i = 1;
         for (String tc : tests) {
            writer.println("@Test");
            writer.println("public void test" + i + "() {");
            String pattern = Utilities.runFSM(FSM, 1, tc, "");
            if (pattern.contains("yes")) {
              writer.println("assertTrue(Bond.bondRegex(\""+tc+"\"));");
            } else {
              writer.println("assertFalse(Bond.bondRegex(\""+tc+"\"));");
            }
            writer.println("}");
            i++;
         }
      	 ...
      	 System.out.println("Error creating test class");
      }
